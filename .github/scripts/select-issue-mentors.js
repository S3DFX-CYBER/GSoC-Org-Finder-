...
const claimant = canonicalizeMentor(process.env.ISSUE_CLAIMANT);
const excluded = new Set(
  safeParseArray('EXCLUDED_MENTORS').map(canonicalizeMentor)
);
...
for (const username of mentors) {
  ...
  if (username === claimant || excluded.has(username) || (recencyDays !== null && recencyDays > 60)) {
...
// Ensure deterministic seed for a given issue (e.g. PR open) unless it's a re-ping
// If we want different results on re-ping, we incorporate time or exclude them. 
// Since we pass EXCLUDED_MENTORS, they are already filtered out above.
const seed = parseInt(issueNumber, 10) + excluded.size * 1337;

const candidates = scoredCandidates.map(c => c.username);
const weights = scoredCandidates.map(c => c.score);

const selected = weightedRandomSelect(candidates, weights, maxMentors, seed);
