...
function applyFilters(){
  ...
  let res=ORGS.filter(o=>{
    ...
    if(pills.size>0){let m=false;const tagTokens=o.tags.flatMap(tag=>tag.toLowerCase().split(/\s+/));pills.forEach(p=>{if(tagTokens.includes(p))m=true});if(!m)return false}
