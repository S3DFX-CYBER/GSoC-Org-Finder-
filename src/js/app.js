...
function updateCountdown() {
  ...
  if (now >= OPEN_DATE.getTime() && now < CLOSE_DATE.getTime()) {
    ...
    banner.style.background =
      'linear-gradient(135deg,rgba(0,135,90,.07),rgba(0,135,90,.12))';
    banner.style.borderBottomColor = 'rgba(0,135,90,.3)';
    banner.style.color = 'var(--green)';
  } else if (now >= CLOSE_DATE.getTime()) {
    banner.innerHTML =
      '<span>🎉 GSoC 2026 applications have closed. Stay tuned for accepted orgs!</span>';
...
const AN = {
  ...
  trackVisit() {
    ...
    this.s('daily', daily);
    if (!sessionStorage.getItem('gaf_s'))
      sessionStorage.setItem('gaf_s', Date.now());
  },
  trackSearch(t) {
    if (t.length > 1) {
      this.inc('searches');
  ...
  topCats() {
    return Object.entries(this.g('cats', {}))
      .sort((a, b) => b[1] - a[1])
      .slice(0, 6);
  },
  topOrgs() {
    return Object.entries(this.g('orgs', {}))
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5);
  },
  topTerms() {
...
function openAnalytics() {
  document.getElementById('aTot').textContent = AN.g(
    'total',
    0
  ).toLocaleString();
  ...
  document.getElementById('catChart').innerHTML = tc.length
    ? tc
        .map(
          ([c, n]) =>
            `<div class="bar-row"><span class="bar-lbl">${escapeHtml(catLabel(c))}</span><div class="bar-track"><div class="bar-fill" style="width:${Math.round((n / mx) * 100)}%"></div></div><span class="bar-val">${escapeHtml(String(n))}</span></div>`
        )
        .join('')
  ...
  document.getElementById('orgChart').innerHTML = to.length
    ? to
        .map(
          ([o, n]) =>
            `<div class="bar-row"><span class="bar-lbl" style="font-size:10px">${escapeHtml(o.length > 16 ? o.slice(0, 16) + '…' : o)}</span><div class="bar-track"><div class="bar-fill" style="width:${Math.round((n / mo) * 100)}%"></div></div><span class="bar-val">${escapeHtml(String(n))}</span></div>`
        )
        .join('')
  ...
  document.getElementById('srchTerms').innerHTML = tt.length
    ? tt
        .map(
          ([t, c], i) =>
            `<span class="sch ${i < 3 ? 'hot' : ''}">${escapeHtml(t)} (${escapeHtml(String(c))})</span>`
        )
        .join('')
...
function closeAnEvent(e) {
  if (e.target === document.getElementById('anBg')) closeAn();
}
function closeAn() {
...
function cleanCache() {
  ...
  for (const key in cache) {
    ...
    if (
      ...
      Number.isNaN(entry.ts) ||
      now - entry.ts > ONE_DAY
    ) {
      delete cache[key];
      changed = true;
    }
...
async function checkAPI() {
  try {
    ...
    if (r.ok) {
      ...
      document.getElementById('apiStrong').textContent =
        '✓ GitHub API Connected';
      document.getElementById('apiText').textContent =
        'Live stats (stars, forks, good first issues) available for all visitors.';
      ...
      document.getElementById('apiText').textContent =
        'Add GITHUB_TOKEN in Vercel dashboard and redeploy.';
    }
  } catch {
    document.getElementById('apiStrong').textContent = '○ Running Locally';
    document.getElementById('apiText').textContent =
      'Deploy to Vercel for live GitHub stats.';
  }
}
...
// ══════════════════════════════════════════════
function yCls(y) {
  return y >= 8 ? 'veteran' : y >= 4 ? 'experienced' : 'newcomer';
}
function yLbl(y) {
  return y >= 8 ? '🏆 Veteran' : y >= 4 ? '⭐ Experienced' : '🌱 Newcomer';
}
function yBdg(y) {
  return y >= 8 ? 'bv' : y >= 4 ? 'be' : 'bn';
}
function cLbl(c) {
  return c === 'hot' ? '🔥 High' : c === 'moderate' ? '🟡 Moderate' : '😎 Low';
}
function cBdg(c) {
  return c === 'hot' ? 'bh' : c === 'moderate' ? 'bm' : 'bc';
}
function aLbl(a) {
  return a === 'active'
    ? '⚡ Active'
...
function syncCompareURL() {
  ...
  const compareItems = [...compareSet]
    .map((idx) => ORGS[idx])
    .filter(Boolean)
    .map((org) => slugifyOrgName(org.name));

  if (!compareItems.length) {
    params.delete('compare');
  } else {
    params.set('compare', compareItems.join(','));
  }
...
function restoreCompareFromURL() {
  ...
  slugs.forEach((slug) => {
    const idx = ORGS.findIndex((org) => slugifyOrgName(org.name) === slug);

    if (idx !== -1 && compareSet.size < 3) {
      compareSet.add(idx);
    }
  });
...
function toggleCompare(idx, e) {
  ...
  if (compareSet.has(idx)) {
    ...
    if (compareSet.size >= 3) {
      showCompareToast('Max 3 orgs for comparison');
      return;
    }
...
function closeCompareEv(e) {
  if (e.target === document.getElementById('compareBg')) closeCompare();
}
...
function renderCompareTable() {
  ...
  const rows = [
    ...
    {
      label: 'Competition',
      vals: arr.map((o) => cLbl(o.competition)),
      scores: arr.map((o) => compScore[o.competition]),
      type: 'scored',
      best: 'low',
    },
    {
      label: 'Stars ⭐',
      vals: arr.map((o) => (o._gh ? fmt(o._gh.stars) : '—')),
      scores: arr.map((o) => o._gh?.stars || 0),
      type: 'scored',
      best: 'high',
    },
    {
      label: 'Forks',
      vals: arr.map((o) => (o._gh ? fmt(o._gh.forks) : '—')),
      scores: arr.map((o) => o._gh?.forks || 0),
      type: 'scored',
      best: 'high',
    },
    {
      label: 'Open Issues',
      vals: arr.map((o) => (o._gh ? fmt(o._gh.issues) : '—')),
      scores: arr.map((o) => o._gh?.issues || 0),
      type: 'scored',
      best: 'low',
    },
    {
      label: 'Last Commit',
      vals: arr.map((o) => (o._gh ? o._gh.lastCommit : '—')),
      type: 'text',
    },
    {
      label: 'Good 1st Issues',
      vals: arr.map((o) =>
        o._gh?.gfi !== null && o._gh?.gfi !== undefined ? fmt(o._gh.gfi) : '—'
      ),
      scores: arr.map((o) => o._gh?.gfi || 0),
      type: 'scored',
      best: 'high',
    },
    {
      label: 'Languages',
      vals: arr.map((o) => o.tags.slice(0, 3).join(', ')),
      type: 'text',
    },
  ];
  ...
  for (const row of rows) {
    ...
    if (row.type === 'bar') {
      ...
      row.type === 'scored' &&
      row.scores &&
      row.scores.some((s) => s > 0)
    ) {
      const mx = Math.max(...row.scores),
        mn = Math.min(...(row.scores.filter((s) => s > 0) || [0]));
      cells = row.vals
        .map((v, i) => {
      ...
      cells = row.vals
        .map((v) => `<td class="cmp-val">${escapeHtml(String(v))}</td>`)
        .join('');
    }
...
function showCompareToast(msg) {
  ...
  if (!t) {
    ...
    t.id = 'compareToast';
    t.style.cssText =
      'position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:var(--ink);color:var(--bg);padding:10px 18px;border-radius:8px;font-size:12px;font-weight:700;z-index:999;transition:opacity .3s;white-space:nowrap';
...
function applyFilters() {
  ...
  const lang =
    document.getElementById('langFilter')?.value?.toLowerCase() || '';
  ...
  const res = ORGS.filter((o) => {
    ...
    if (chips.has('active') && (!o._gh || o._gh.activity !== 'active'))
      return false;
...
function toggleBookmark(event, orgIdx) {
  ...
  document.dispatchEvent(
    new CustomEvent('bookmarkChanged', { detail: { name: orgName } })
  );
}
    ...
    .map((o, i) => {
      ...
      <div class="card-header-row">
        ...
        <div class="org-logo-info">
          ...
          ${
            repoHref
              ? `<a class="card-repo-link" href="${escapeHtml(repoHref)}" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation()" title="${escapeHtml(repoHref)}">
            ${
...
function updateStats() {
  ...
  document.getElementById('veteranStat').textContent = ORGS.filter(
    (o) => o.years >= 8
  ).length;
  document.getElementById('newcomerStat').textContent = ORGS.filter(
    (o) => o.years <= 3
  ).length;
...
const GRID_COLS = () => {
  ...
  for (let i = 1; i < g.children.length; i++) {
    if (Math.abs(g.children[i].getBoundingClientRect().top - firstRect.top) < 5)
      cols++;
    else break;
  }
  return cols;
...
function resetFilters() {
  [
    ...
    if (e)
      e.value =
        id === 'categoryFilter' || id === 'complexityFilter' ? 'all' : '';
  });
...
function openModal(idx) {
  ...
  document.getElementById('mCat').innerHTML =
    `<span class="cat-pill ${catBdg(o.cat)}">${escapeHtml(catLabel(o.cat))}</span>`;
  ...
  document.getElementById('ghGFI').textContent =
    gh?.gfi !== null && gh?.gfi !== undefined ? fmt(gh.gfi) : '—';
  document.getElementById('mFetchBtn').textContent = gh
    ? '↻ Refresh'
    : 'Fetch Live Data';
  document.getElementById('mTags').innerHTML = o.tags
    .map((t) => `<span class="m-tag">${escapeHtml(t)}</span>`)
    .join('');
  document.getElementById('mFit').innerHTML = o.fit
    .map((f) => `<span class="m-tag">${escapeHtml(f)}</span>`)
    .join('');
  let tl = '';
  ...
  if (mLinkEl && o.github) {
    ...
    mLinkEl.href = isUmbrella
      ? owner
        ? `https://github.com/${owner}`
        : ''
      : githubUrlFromValue(o.github);
    mLinkEl.textContent = isUmbrella
      ? 'View GitHub Org →'
      : 'View Repository →';
  }
  ...
  for (let i = 0; i < orgsWithGithub.length; i += BATCH) {
    ...
    await Promise.all(
      batch.map(async (o) => {
        try {
          const r = await fetch(
            `${API}?repo=${encodeURIComponent(o.github)}&gfi=1&issues=1`
          );
          if (!r.ok) return;
    ...
    if (fpStatus)
      fpStatus.textContent = `Checking ${done} / ${orgsWithGithub.length} orgs`;
...
async function loadCachedIssues() {
  ...
  try {
    ...
    allIssues = data.issues.map((issue) => {
      const key =
        issue.github?.toLowerCase() ||
        issue.repo?.toLowerCase() ||
        issue.org?.toLowerCase();
      const orgMeta =
        orgByGithub.get(key) || orgByName.get(issue.org?.toLowerCase());
...
function filterIssues() {
  ...
  filteredIssues = allIssues.filter((iss) => {
    ...
    if (
      search &&
      !iss.title.toLowerCase().includes(search) &&
      !iss.org.toLowerCase().includes(search)
    )
      return false;
    return true;
  });
...
function renderIssues() {
  ...
  if (!filteredIssues.length) {
    ...
    shownIssues = Math.min(
      shownIssues + ISSUES_PAGE_SIZE,
...
function renderIssueCard(iss) {
  const langTags = iss.orgTags
    .slice(0, 2)
    .map((t) => `<span class="issue-label lang">${escapeHtml(t)}</span>`)
    .join('');
  const gfiNames = ['good first issue', 'good-first-issue'];
  const otherLabels = iss.labels
    .filter((l) => !gfiNames.includes(String(l).toLowerCase()))
    .slice(0, 2)
    .map(
      (l) =>
        `<span class="issue-label" style="background:rgba(107,33,168,.06);color:var(--purple);border:1px solid rgba(107,33,168,.2)">${escapeHtml(l)}</span>`
    )
    .join('');
  const safeHref = validateIdeasUrl(iss.url);
  const imgSrc = validateIdeasUrl(iss.logo);
  const hrefStart = safeHref
    ? `<a class="issue-card" href="${escapeHtml(safeHref)}" target="_blank" rel="noopener noreferrer">`
...
function showMoreIssues() {
  const container = document.getElementById('issuesContainer');
  const next = filteredIssues.slice(
    shownIssues,
    shownIssues + ISSUES_PAGE_SIZE
  );
  shownIssues += next.length;
  container
    .querySelector('.issues-grid')
    .insertAdjacentHTML('beforeend', next.map(renderIssueCard).join(''));
...
ORGS.forEach((o) => {
  if (o.github && cache[o.github]) o._gh = cache[o.github];
});
showSkeletons();
updateStats();
renderSelectedLanguages();
