...
const AN = {
    ...
    sessionTime() {
        ...
        const sec = Math.floor((Date.now() - Number.parseInt(s, 10)) / 1000);
...
async function fetchModalGH() {
    ...
    if (d) {
        ...
        if (gfi !== null) {
            ...
            if (cells?.[3]) cells[3].textContent = gfiTxt;
        }
        applyFilters();
        renderCompareTable();
...
function renderCompareTableRow(row) {
    if (row.type === 'bar') {
        const mx = Math.max(...row.vals);
        return row.vals.map(v => {
    ...
    if (row.type === 'scored' && row.scores?.some(s => s > 0)) {
        const mx = Math.max(...row.scores);
        const mn = Math.min(...row.scores.filter(s => s > 0) || [0]);
        return row.vals.map((v, i) => {
            const s = row.scores[i];
            let cls = 'cmp-val';
            if (s > 0) {
                if (row.best === 'high' && s === mx) cls = 'cmp-best';
                else if (row.best === 'high' && s === mn) cls = 'cmp-worst';
                else if (row.best !== 'high' && s === mn) cls = 'cmp-best';
                else if (row.best !== 'high' && s === mx) cls = 'cmp-worst';
            }
            return `<td class="${cls}">${escapeHtml(String(v))}</td>`;
        }).join('');
    }
    return row.vals.map(v => `<td class="cmp-val">${escapeHtml(String(v))}</td>`).join('');
}
...
function renderCompareTable() {
    ...
    const theadHeaders = arr.map(o => {
        const displayName = o.name.length > 22 ? o.name.slice(0, 22) + '…' : o.name;
        return '<th>' + escapeHtml(displayName) + '</th>';
    }).join('');
    const thead = '<tr><th>Metric</th>' + theadHeaders + '</tr>';

    let tbody = '';
    for (const row of rows) {
        const cells = renderCompareTableRow(row);
...
function passChipFilters(o) {
    ...
    if (chips.has('active') && o._gh?.activity !== 'active') return false;
    if (chips.has('bookmarked') && !isBookmarked(o.name)) return false;
    return true;
}

function passOrgFilterFilters(o, cat, compF, langLabel, search) {
    if (!passBasicFilters(o, cat, compF, langLabel, search)) return false;
    if (!passChipFilters(o)) return false;
    return true;
}

function sortOrgResults(res, search, sort) {
    if (!search) {
...
function applyFilters() {
    ...
    const langLabel = lang ? (Object.keys(LANGUAGE_MAP).find(label => label.toLowerCase() === lang) || lang) : '';
    const res = ORGS.filter(o => passOrgFilterFilters(o, cat, compF, langLabel, search));

    sortOrgResults(res, search, sort);
...
function renderGrid(orgs) {
    ...
    g.innerHTML = orgs.map((o, i) => {
      ...
      <span class="gh-s">🕐 <b>${escapeHtml(String(o._gh.lastCommit))}</b></span>
        ...
        let repoLinkHtml = '';
        if (repoHref) {
            const isUmbrella = UMBRELLA_ORGS.has(o.name) || !repoPath.includes('/');
            const repoIcon = isUmbrella ?
            ...
            repoLinkHtml = `<a class="card-repo-link" href="${escapeHtml(repoHref)}" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation()" title="${escapeHtml(repoHref)}">
            ${repoIcon}
            ${repoLinkLabel(o)}
          </a>`;
        }
...
function navigateDirection(dir, n, cols) {
    if (focusedIdx < 0) {
        focusedIdx = 0;
        return;
    }
    if (dir === 'Right') focusedIdx = Math.min(focusedIdx + 1, n - 1);
    else if (dir === 'Left') focusedIdx = Math.max(focusedIdx - 1, 0);
    else if (dir === 'Down') focusedIdx = Math.min(focusedIdx + cols, n - 1);
    else if (dir === 'Up') focusedIdx = Math.max(focusedIdx - cols, 0);
}

function handleNavigationKeys(e, n, cols) {
    if (['ArrowRight', 'ArrowLeft', 'ArrowDown', 'ArrowUp'].includes(e.key)) {
        e.preventDefault();
        navigateDirection(e.key.replace('Arrow', ''), n, cols);
        scrollToFocused();
        renderGrid(filteredOrgs);
        return true;
    }

    if (e.key === 'Enter' && focusedIdx >= 0 && focusedIdx < n) {
        openModal(ORGS.indexOf(filteredOrgs[focusedIdx]));
        return true;
    }

    if ((e.key === 'c' || e.key === 'C') && focusedIdx >= 0 && focusedIdx < n) {
        e.preventDefault();
...
function renderModalTimeline(o) {
    ...
    for (let y = o.firstYear; y <= 2026; y++) {
        const cur = y === 2026;
        const color = cur ? 'var(--orange)' : 'var(--ink3)';
        const weight = cur ? 700 : 400;
        const check = cur ? '⭐' : '✓';
        tl += `<span style="margin-right:10px;color:${color};font-weight:${weight}">${check} ${escapeHtml(String(y))}</span>`;
    }
    document.getElementById('mTimeline').innerHTML = tl;
}

function renderModalContent(o) {
    ...
    let yearColor = 'var(--purple)';
    if (o.years >= 8) yearColor = '#C2410C';
    else if (o.years >= 4) yearColor = 'var(--blue)';

    let compEmoji = '😎';
    if (o.competition === 'hot') compEmoji = '🔥';
    else if (o.competition === 'moderate') compEmoji = '🟡';

    let gfiDisplay = '—';
    if (o._gh?.gfi !== null && o._gh?.gfi !== undefined) {
        gfiDisplay = fmt(o._gh.gfi);
    }

    const cc = { hot: 'var(--red)', moderate: '#92600A', chill: 'var(--green)' };
    document.getElementById('mMetrics').innerHTML = `
    <div class="mc"><div class="mv" style="color:${yearColor}">${escapeHtml(String(o.years))}</div>
    <div class="ml">GSoC Years</div><div class="prog"><div class="prog-fill" style="width:${Math.min(o.years / 11 * 100, 100)}%;background:${yearColor}"></div></div></div>
    <div class="mc"><div class="mv" style="color:${cc[o.competition]}">${compEmoji}</div><div class="ml">${escapeHtml(String(cLbl(o.competition)))}</div></div>
    <div class="mc"><div class="mv" style="color:var(--orange)">${escapeHtml(String(o.firstYear))}</div><div class="ml">First Year</div></div>
    <div class="mc"><div class="mv" style="color:var(--green)">${escapeHtml(gfiDisplay)}</div><div class="ml">Good 1st Issues</div></div>`;

    document.getElementById('ghStars').textContent = o._gh ? fmt(o._gh.stars) : '—';
    document.getElementById('ghForks').textContent = o._gh ? fmt(o._gh.forks) : '—';
    document.getElementById('ghIssues').textContent = o._gh ? fmt(o._gh.issues) : '—';
    document.getElementById('ghCommit').textContent = o._gh ? o._gh.lastCommit : '—';
    document.getElementById('ghGFI').textContent = gfiDisplay;
    document.getElementById('mFetchBtn').textContent = o._gh ? '↻ Refresh' : 'Fetch Live Data';
    ...
    renderModalTimeline(o);
}

function updateModalLinks(o) {
    ...
    if (mLinkEl && o.github) {
        ...
        if (isUmbrella) {
            mLinkEl.href = owner ? `https://github.com/${owner}` : '';
            mLinkEl.textContent = 'View GitHub Org →';
        } else {
            mLinkEl.href = githubUrlFromValue(o.github);
            mLinkEl.textContent = 'View Repository →';
        }
    }
    ...
    if (mIdeasEl) {
        if (validatedUrl) {
            mIdeasEl.style.display = 'inline-flex';
    ...
    if (o.github && (o._gh?.gfi === null || o._gh?.gfi === undefined)) {
        ...
        fetchGFI(o.github).then(gfi => {
            if (gfi === null) {
                ...
                const cells = document.getElementById('mMetrics')?.querySelectorAll('.mv');
                if (cells?.[3]) cells[3].textContent = fmt(gfi);
                renderGrid(filteredOrgs);
                renderCompareTable();
            }
        });
...
function renderIssueCard(iss) {
    ...
    const gfiNames = new Set(['good first issue', 'good-first-issue']);
    const otherLabels = iss.labels.filter(l => !gfiNames.has(String(l).toLowerCase())).slice(0, 2)
