...
async function fetchGFI(repo) {
  ...
  if (hit && Date.now() - hit.ts < 3600000 && hit.count !== null && hit.count !== undefined)
    return hit.count;
  try {
...
function aBdg(a) {
  return a === 'active' ? 'bac' : a === 'moderate' ? 'bam' : a === 'low' ? 'bal' : 'bna';
}
function catLabel(c) {
  return (
    {
...
function repoUrl(o) {
  ...
  if (UMBRELLA_ORGS.has(o.name) || !o.github.includes('/')) return `https://github.com/${owner}`;
  return `https://github.com/${o.github}`;
}
function repoLinkLabel(o) {
  if (!o.github) return '';
  const owner = o.github.includes('/') ? o.github.split('/')[0] : o.github;
  if (UMBRELLA_ORGS.has(o.name) || !o.github.includes('/')) return owner + ' (org)';
  return o.github;
}
...
function resetFilters() {
  ['searchInput', 'catFilter', 'langFilter', 'yearsFilter', 'compFilter'].forEach((id) => {
...
async function loadCachedIssues() {
  ...
  try {
    ...
    allIssues = data.issues.map((issue) => {
      const key =
        issue.github?.toLowerCase() || issue.repo?.toLowerCase() || issue.org?.toLowerCase();
