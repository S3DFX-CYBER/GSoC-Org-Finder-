...
function openAnalytics() {
  ...
  document.getElementById("catChart").innerHTML = tc.length
    ? tc
        .map(
          ([c, n]) =>
            `<div class="bar-row"><span class="bar-lbl">${catLabel(c)}</span><div class="bar-track"><div class="bar-fill" style="width:${Math.round((n / mx) * 100)}%"></div></div><span class="bar-val">${n}</span></div>`,
        )
        .join("")
  ...
  document.getElementById("orgChart").innerHTML = to.length
    ? to
        .map(
          ([o, n]) =>
            `<div class="bar-row"><span class="bar-lbl" style="font-size:10px">${o.length > 16 ? o.slice(0, 16) + "…" : o}</span><div class="bar-track"><div class="bar-fill" style="width:${Math.round((n / mo) * 100)}%"></div></div><span class="bar-val">${n}</span></div>`,
        )
        .join("")
  ...
  document.getElementById("srchTerms").innerHTML = tt.length
    ? tt
        .map(
          ([t, c], i) =>
            `<span class="sch ${i < 3 ? "hot" : ""}">${t} (${c})</span>`,
        )
        .join("")
...
function applyFilters() {
  ...
  let res = ORGS.filter((o) => {
    ...
    if (yearsF) {
      const yc = yCls(o.years);
      if (yearsF !== yc) return false;
    }
    if (compF && o.competition !== compF) return false;
    if (pills.size > 0) {
      let m = false;
      pills.forEach((p) => {
        if (txt.includes(p)) m = true;
      });
      if (!m) return false;
    }
  ...
  if (sort === "alpha") res.sort((a, b) => a.name.localeCompare(b.name));
    ...
    res.sort((a, b) => (b._gh?.stars || 0) - (a._gh?.stars || 0));
  else if (sort === "gfi")
    res.sort((a, b) => (b._gh?.gfi || 0) - (a._gh?.gfi || 0));
...
function renderGrid(orgs) {
  ...
  if (!orgs.length) {
    g.innerHTML = `<div class="empty"><div class="empty-icon">🔍</div><h3>No matches found</h3><p>Try removing some filters.</p></div>`;
    return;
  }
  g.innerHTML = orgs
    .map((o, i) => {
      ...
      <div class="card-header-row">
        ...
        <div class="org-logo-info">
          ...
          ${
            repoHref
              ? `<a class="card-repo-link" href="${repoHref}" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation()" title="${repoHref}">
            ${
...
function togglePill(el) {
  ...
  if (el.classList.contains("active")) pills.add(l);
  else pills.delete(l);
  applyFilters();
}
const chipCls = {
...
function renderIssueCard(iss) {
  const langTags = iss.orgTags
    .slice(0, 2)
    .map((t) => `<span class="issue-label lang">${t}</span>`)
    .join("");
  ...
  const otherLabels = iss.labels
    ...
    .map(
      (l) =>
        `<span class="issue-label" style="background:rgba(107,33,168,.06);color:var(--purple);border:1px solid rgba(107,33,168,.2)">${l}</span>`,
    )
    .join("");
